...
var ProductOptionWhere = struct {
	...
	ProductID: whereHelperint64{field: "`product_option`.`product_id`"},
	Position:  whereHelperint{field: "`product_option`.`position`"},
	Name:      whereHelperstring{field: "`product_option`.`name`"},
}
...
type (
	...
	// This should almost always be used instead of []ProductOption.
...
func FindProductOption(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*ProductOption, error) {
	...
	if err = productOptionObj.doAfterSelectHooks(ctx, exec); err != nil {
		return productOptionObj, err
	}
