...
var InventoryItemWhere = struct {
	...
	RequiresShipping:     whereHelperbool{field: "`inventory_item`.`requires_shipping`"},
	Sku:                  whereHelpernull_String{field: "`inventory_item`.`sku`"},
	Tracked:              whereHelperbool{field: "`inventory_item`.`tracked`"},
...
type (
	...
	// This should almost always be used instead of []InventoryItem.
...
func FindInventoryItem(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*InventoryItem, error) {
	...
	if err = inventoryItemObj.doAfterSelectHooks(ctx, exec); err != nil {
		return inventoryItemObj, err
	}
