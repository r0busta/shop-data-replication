...
func testLocationToManyInventoryLevels(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadInventoryLevels(ctx, tx, false, (*[]*Location)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testLocationToManyAddOpInventoryLevels(t *testing.T) {
	var err error
...
var (
	locationDBTypes = map[string]string{`ID`: `bigint`, `Active`: `tinyint`, `Address1`: `varchar`, `Address2`: `varchar`, `City`: `varchar`, `CreatedAt`: `datetime`, `Name`: `varchar`, `Phone`: `varchar`, `UpdatedAt`: `datetime`, `Zip`: `varchar`, `CountryCode`: `varchar`, `ProvinceCode`: `varchar`}
	_               = bytes.MinRead
)
...
func testLocationsUpsert(t *testing.T) {
	...
	if len(mySQLLocationUniqueColumns) == 0 {
