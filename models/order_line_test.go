...
func testOrderLineToOneCustomerOrderUsingOrder(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var local OrderLine
	var foreign CustomerOrder

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, orderLineDBTypes, false, orderLineColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize OrderLine struct: %s", err)
	}
	if err := randomize.Struct(seed, &foreign, customerOrderDBTypes, false, customerOrderColumnsWithDefault...); err != nil {
...
func testOrderLineToOneProductUsingProduct(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := OrderLineSlice{&local}
	if err = local.L.LoadProduct(ctx, tx, false, (*[]*OrderLine)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testOrderLineToOneSetOpCustomerOrderUsingOrder(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, customerOrderDBTypes, false, strmangle.SetComplement(customerOrderPrimaryKeyColumns, customerOrderColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, customerOrderDBTypes, false, strmangle.SetComplement(customerOrderPrimaryKeyColumns, customerOrderColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*CustomerOrder{&b, &c} {
...
func testOrderLineToOneSetOpProductUsingProduct(t *testing.T) {
	var err error
...
func testOrderLineToOneRemoveOpProductUsingProduct(t *testing.T) {
	var err error
...
var (
	orderLineDBTypes = map[string]string{`ID`: `bigint`, `OrderID`: `bigint`, `ProductID`: `bigint`, `VariantID`: `bigint`, `Sku`: `varchar`, `Name`: `varchar`, `Title`: `varchar`, `VariantTitle`: `varchar`, `Vendor`: `varchar`, `Quantity`: `int`, `Price`: `decimal`, `TotalDiscount`: `decimal`}
	_                = bytes.MinRead
)
...
func testOrderLinesUpsert(t *testing.T) {
	...
	if len(mySQLOrderLineUniqueColumns) == 0 {
